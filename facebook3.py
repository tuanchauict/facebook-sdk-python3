...
class GraphAPI(object):
	...
	def put_photo(self, image, message=None, album_id=None, **kwargs):
		...
		req = urllib.request.Request("https://graph.facebook.com/%s/photos" % object_id, data=body)
	...
	def request(self, path, args=None, post_args=None):
		...
		url = "https://graph.facebook.com/" + path + "?" + urllib.parse.urlencode(args)

		response = self.fetch_url(url, post_args)
...
def get_app_access_token(application_id, application_secret):
	...
	try:
		result = file.read()
		if result:
			result = result.decode('utf8').split("=")[1]
	finally:
		file.close()
